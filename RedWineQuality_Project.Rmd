...
outlierKD <- function(dt, var, t) {
  var_name <- eval(substitute(var),eval(dt))
  na1 <- sum(is.na(var_name))
  m1 <- mean(var_name, na.rm = T)
  par(mfrow=c(1, 2), oma=c(0,0,3,0))
  #boxplot(var_name, main="With outliers")
  hist(var_name, main="With outliers", xlab=NA, ylab=NA, border = "steelblue4")
  outlier <- boxplot.stats(var_name)$out
  mo <- mean(outlier)
  var_name <- ifelse(var_name %in% outlier, NA, var_name)
  #boxplot(var_name, main="Without outliers")
  hist(var_name, main="Without outliers", xlab=NA, ylab=NA, border = "steelblue4")
...
outlierKD(red_wine, residual.sugar, "Residual Sugar Outlier Check")
```
...
outlierKD(red_wine, chlorides, "Chlorides Outlier Check")
...
#Display histograms with and without outliers for chlorides
outlierKD(red_wine, citric.acid, "Citric Acid Outlier Check")
...
#Create scatter plot and boxplot for alcohol and quality
b1p <- ggplot(aes(x = quality, y = alcohol), data = subset(red_wine, 
                                                           alcohol <= 14)) +
  geom_point()

b2p <- ggplot(aes(x =quality_factor, y = alcohol, colour = rating), 
              data = subset(red_wine, alcohol <= 14)) +
  geom_boxplot()

#Display plots
grid.arrange(b1p, b2p, ncol = 2)
```
...
b3p <- ggplot(aes(x = quality, y = volatile.acidity), data = red_wine) +
  geom_point()
b4p <- ggplot(aes(x =quality_factor, y = volatile.acidity, colour = rating), 
              data = red_wine) +
  geom_boxplot()

#Display plots
grid.arrange(b3p, b4p, ncol = 2)
```
...
#Display plots
grid.arrange(ta1q, ta2q, ta3q, ncol= 1)
...
are also supported by the plots above. If the relationship between fixed acidity and citric 
acid, pH and density is clear, no clear relationship with quality can be seen.
...
grid_arrange_shared_legend <- function(..., ncol = length(list(...)),nrow = 1, 
  ...
  combined <- switch(position,
                     "bottom" = arrangeGrob(titleGrob,
                                            ...
                                            legend,
                                            ncol = 1, 
                                            heights = unit.c(lheight, unit(1, "npc") - lheight - lheight, lheight)),
                     "right" = arrangeGrob(titleGrob,
                                           do.call(arrangeGrob, gl),
                                           legend, 
                                           ncol = 2,
                                           widths = unit.c(lheight, unit(1, "npc") - lwidth - lwidth, lwidth)))
...
grid_arrange_shared_legend(pl21, pl22, ncol = 2, nrow = 1, title = "Characteristics with highest impact on wine quality")
```
